...
pub fn select<'a, 'b>(s: &Select<'a>, db: &'b Db) -> Result<'a, SelectResult<'b>> {
  unsafe {
    ...
    // if one of the CmpOp is not Ne and their types are the same (ignore size), it will be put into `cross_cols`, and we can use binary search to locate RHS
    ...
    for cond in &s.where_ {
      ...
      if let Some(((mut tp_r, mut ci_r, mut idx_r), mut op)) = {
        ...
        one_preds.get_unchecked_mut(idx_l).push(one_predicate(cond, tp_l)?);
      ...
      if let Some((idx_l, (op, ci_r, ci_l))) = (0..idx_r).filter_map(|idx_l| at!(cross_cols,idx_r, idx_l).map(|x| (idx_l, x))).next() {
        ...
        for old_idx in 0..(final_.len() / tbl_num) {
          ...
          let rg = match ci_r.ty.ty {
            ...
            VarChar => rs.equal_range_by(|&r| str_from_db(r.add(off_r)).cmp(str_from_db(l))),
          };
          let rg = match op {
            Lt => 0..rg.start, Le => 0..rg.end, Ge => rg.start..rs.len(), Gt => rg.end..rs.len(), Eq => rg, Ne => debug_unreachable!(),
          };
          join!(old_row, rg);
        }
      } else {
