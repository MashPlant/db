...
本项目使用rust实现。执行`cargo run --bin db --release`运行数据库repl，执行`cargo test -p tests --release`进行测试，执行`make`进行代码覆盖率测试。要求nightly版本的rust编译器，版本越新越好；为了执行代码覆盖率测试，需先安装`cargo-tarpaulin`(安装方法为`cargo install cargo-tarpaulin`)，并且装有makefile中指定的浏览器。
...
我的数据库项目按照rust的crate为边界划分逻辑，crate间的依赖关系为：
...
每种页的物理布局在`physics` crate中描述，代码本身就很清楚了，无需多用自然语言描述。值得注意的是rid的实现，它用于在前一个文件中定位字段。使用一个32位无符号整数来描述rid，其中高23位表示页号，低9位表示页内编号，因此前一个文件的最大容量为$2^{23}*8192\text{B}=64\text{GB}$，每个数据页最多有$2^9=512$个数据项。

后一个文件中因为没有页的概念，所以直接用一个32位无符号整数来进行寻址即可，寻址的粒度(即变长字段空间分配的粒度)是$32\text{B}$，所以后一个文件的最大容量为$2^{32}*32\text{B}=128\text{GB}$。空间的分配采用简单的first-fit算法。因为实现的复杂性，`test` crate中专门设计了对变长字段空间分配的测试，用很大规模的输入数据测试了空间分配的正确性。
...
B+树的实现没有什么可以仔细解释的地方，就是完整的实现了所有相关的逻辑，例如插入时节点的分裂和删除时节点的合并。此外还做了一点优化，在一个节点内部搜索时可以使用二分查找，由于每个页内的表项数目往往是较大的，因此二分查找的确有一定的加速效果。
...
主键和外键约束的实现很大程度上依赖于索引，所以我设定相关的数据约束都不能作用于`varchar`类型，因为我的索引实现不支持它。单一主键的唯一性我采用查询索引来实现，复合主键的唯一性我采用散列表来实现，这个散列表是在修改数据时临时构造的，所以可能会有很大的开销。外键要求一定要引用一个保证了唯一性的字段，包括标注`unique`的字段和单一主键。**不支持对外键起名字，不支持复合外键**。

alter表操作我实现了删除一个列和在末尾添加一个列。一般来说这是一个相当昂贵的操作，我的实现中，需要先把所有的新格式的数据都插入数据库，然后再删除全部的旧数据，然后更新全部索引。

## 查询解析部分

我实现的sql语法与`parser 文法规则`文档中要求的基本一致，但也有一定的区别，支持的语法略少一些，例如不支持对外键起名字，不支持复合外键等。**parser层面支持的语法，底层的实现也支持，反之则不支持**。
...
各种查询都用到的一个基础设施是对于单个表的过滤。这一步我会尝试使用索引来优化，优化也很简单，即对于列和常数的比较，可以直接通过遍历B+树的特定区间来代替。这里我支持了模糊查询，底层是使用rust的正则表达式来实现的。

`select`中，首先收集所有只涉及单个表的判断条件，使用它们对各自的表先过滤一遍，得到表的个数个结果列表。然后使用跨两个表的判断条件来进行两个列表的组合，这里我做了一点优化，如果一个条件，比较的左右项类型相同，而且比较条件不是`!=`，则可以使用排序+二分搜索来进行组合。得到完整的结果之后可以进行聚集操作。经测试对于多个表的连接仍然能有不错的性能。

`insert`中，首先结合命名的列，列的默认值和插入值给出实际插入的数据，然后申请一个临时的缓冲区，将所数据先写入这个缓冲区，同时进行完整性检查，确认没有任何错误之后，申请新的一行的空间，将缓冲区中的内容写入这一行。为了实现多主键约束的检查，在开始插入之前先构造一个散列表，插入的同时查询和维护散列表，保证不出现重复的多主键，其余的约束的检查一般可以借助索引进行。

`delete`中，直接遍历一遍单个表的过滤结果进行删除即可。删除的同时需要删除对应的索引，为了避免在B+树上进行迭代的同时修改树，`delete`不会采用索引优化。执行删除的时候可能破坏参照完整性，我实现的是restrict策略，即如果发现当前被删除的列中包含被外键引用的字段，则拒绝删除并报错。

`update`中，结合了`insert`的完整性检查的逻辑(复用了`insert`中的代码)与`delete`类似的参照完整性检查之外。除此之外，在这里我实现了更加复杂的表达式的运算，包括基本的四则运算和逻辑运算(支持短路)等，以及表达式的任意组合。
...
因为不支持复合外键，所以不能直接运行给出的测试样例。不过这也是唯一一个不支持的地方，去掉对应的约束语句之后即可正常执行，读入小数据集的全部数据耗时约1秒。
